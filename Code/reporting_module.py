...
def _calc_ci(data_vector, ci = 95, n = 1000):
    """
...
def print_summary_per_lab(data, ci = False):
    ...
    for lab in data['Modality Room'].unique():
        ...
        print(lab + ': n = {:4}'.format(len(data[data['Modality Room'] == lab])) + \
              ...
              (' 95% CI: [' + str(round(lci, 2)) + ' - ' + str(round(uci, 2)) + '] (Gy*cm2)' if ci else '') + \
              # 25 th percentile:
...
def report_exposure_time(data, ci = False):
    ...
    for lab in data['Modality Room'].unique():
        ...
        print(lab + ': n = {:4}'.format(len(data[data['Modality Room'] == lab])) + \
        ', Exposure time: Median - ' + str(round(data[data['Modality Room'] == lab]['F+A Time (s)'].median(), 2)) + ',' +\
        (' 95% CI: [' + str(round(lci, 2)) + ' - ' + str(round(uci, 2)) + '] (s)' if ci else '') + \
        # 25 th percentile:
        ' IQR [' + str(round(data[data['Modality Room'] == lab]['F+A Time (s)'].quantile(0.25), 2)) + \
        ' - ' + str(round(data[data['Modality Room'] == lab]['F+A Time (s)'].quantile(0.75), 2)) + '] (s), ' + \
        'Range (' + str(round(data[data['Modality Room'] == lab]['F+A Time (s)'].min(), 2)) + \
        ' - ' + str(round(data[data['Modality Room'] == lab]['F+A Time (s)'].max(), 2)) + ').')