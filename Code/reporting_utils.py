...
def print_summary_per_lab(data):
    for lab in data['Modality Room'].unique():
        print(lab + ': n = {:4}'.format(len(data[data['Modality Room'] == lab])) + \
              ', DAP: Median - ' + str(round(data[data['Modality Room'] == lab]['DAP Total (Gy*cm2)'].median(), 1)) + ',' +\
              # 25 th percentile:
              ' IQR [' + str(round(data[data['Modality Room'] == lab]['DAP Total (Gy*cm2)'].quantile(0.25), 1)) + \
              ' - ' + str(round(data[data['Modality Room'] == lab]['DAP Total (Gy*cm2)'].quantile(0.75), 1)) + '] (Gy*cm2), ' + \
              'Range (' + str(round(data[data['Modality Room'] == lab]['DAP Total (Gy*cm2)'].min(), 1)) + \
              ' - ' + str(round(data[data['Modality Room'] == lab]['DAP Total (Gy*cm2)'].max(), 1)) + ').')